...
if (!empty($body['delete'])) {
    ...
    $prods['categories'][$cat] = array_values(array_filter($prods['categories'][$cat]??[], fn($f) => $f !== $file));
    @unlink(UPLOAD_DIR . 'products/' . $file);
...
if (!empty($body['images']) && is_array($body['images'])) {
    $prods['categories'][$cat] = array_values(array_map('basename', $body['images']));
    jsonWrite(DATA_DIR . 'pages/products.json', $prods);
